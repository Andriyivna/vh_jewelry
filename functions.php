...
if ( ! function_exists( 'vh_jewelry_setup' ) ) :
	/**
	 ...
	 */
	function vh_jewelry_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on Jewelry, use a find and replace
		 * to change 'vh_jewelry' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'vh_jewelry', get_template_directory() . '/languages' );
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'vh_jewelry_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'vh_jewelry_setup' );
 ...
 */
function vh_jewelry_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'vh_jewelry_content_width', 640 );
}
add_action( 'after_setup_theme', 'vh_jewelry_content_width', 0 );
 ...
 */
function vh_jewelry_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'vh_jewelry' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'vh_jewelry' ),
...
add_action( 'widgets_init', 'vh_jewelry_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'vh_jewelry_scripts' );
...
function vh_jewelry_pagination_bar() {
    global $wp_query;
